...
#Economic Consequences and Health Impact of Weather Events in the United States
...
In this report you can find an investigation of the storm events that have the highest economic consequences and health impact United Statesare. After analysing the database of the National Oceanic and Atmospheric Administration (NOAA) the conclusions are:

* tornadoes are the most dangerous weather event causing high impact on population health. 
* the second event type is the excessive heat. 

In terms of econominc impact:

* flash floods and thunderstorm winds are causing billions of dollars in property damages. 
...
The first step is start loading the specified source files from URL and storing it locally and then read the data into a data frame.
...
Load the data in storm variable
...
Next step is to standardize the data, basically ir requires preprocessing. Event types nor necesarly have a specific format. For instance, there are events with types `Frost/Freeze`,
`FROST/FREEZE` and `FROST\\FREEZE` which obviously refer to the same type of event.
...
##4. Economic Effects of Weather Events
...
###5.1 Health impact of weather events
...
##5.2 Economic impact of weather events
